...
namespace AAS.Architecture.Gateway.Infrastructure
{
    internal sealed class AsyncRoutesMiddleware : IMiddleware
    {
        private static readonly ConcurrentDictionary<string, IConventions> Conventions =
            new ConcurrentDictionary<string, IConventions>(StringComparer.OrdinalIgnoreCase);
        ...
        public async Task InvokeAsync(HttpContext context, RequestDelegate next)
        {
            ...
            {
                var authenticateResult = await context.AuthenticateAsync().WithoutCapturingContext();
            ...
            var message = await _payloadBuilder.BuildFromJsonAsync<object>(context.Request).WithoutCapturingContext();
            var resourceId = Guid.NewGuid().ToString("N");
            if (string.Equals(context.Request.Method, "POST", StringComparison.OrdinalIgnoreCase) && message is JObject jObject)
            {
